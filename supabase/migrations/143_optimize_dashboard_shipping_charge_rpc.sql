...
COMMENT ON FUNCTION public.get_daily_shipping_charge_data(DATE, DATE) IS
  'Returns daily company-borne shipping charges allocated to product and channel by line-level net-sales share. Optimized to filter positive shipping orders early and aggregate lines per order before allocation.';
