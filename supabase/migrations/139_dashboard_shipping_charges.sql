...
CREATE OR REPLACE FUNCTION public.parse_scalev_money_text(p_value TEXT)
RETURNS NUMERIC
LANGUAGE plpgsql
IMMUTABLE
AS $$
...
COMMENT ON FUNCTION public.parse_scalev_money_text(TEXT) IS
  'Parses ScaleV money strings such as "9500.00" or "Rp 9.500" into NUMERIC values.';

DROP FUNCTION IF EXISTS public.get_daily_shipping_charge_data(DATE, DATE);
...
GRANT EXECUTE ON FUNCTION public.get_daily_shipping_charge_data(DATE, DATE) TO authenticated;
