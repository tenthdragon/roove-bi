...
export default function ProductsPage() {
  ...
  useEffect(() => {
    ...
    const cached = getCached<any[]>('daily_product_summary_products', dateRange.from, dateRange.to);
    if (cached) { setData(cached); return; }
    supabase.from('daily_product_summary').select('*').gte('date',dateRange.from).lte('date',dateRange.to)
      .then(({ data:d }) => { const result = d || []; setCache('daily_product_summary_products', dateRange.from, dateRange.to, result); setData(result); });
  }, [dateRange, supabase]);
