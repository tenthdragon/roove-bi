...
export default function MarketingPage() {
  ...
  useEffect(() => {
    ...
    const cachedAds = getCached<any[]>('daily_ads_spend', dateRange.from, dateRange.to);
    const cachedProd = getCached<any[]>('daily_product_summary_mkt', dateRange.from, dateRange.to);
    if (cachedAds && cachedProd) { setAdsData(cachedAds); setProdData(cachedProd); return; }
    Promise.all([
      ...
      const ads = a||[]; const prod = p||[];
      setCache('daily_ads_spend', dateRange.from, dateRange.to, ads);
      setCache('daily_product_summary_mkt', dateRange.from, dateRange.to, prod);
      setAdsData(ads); setProdData(prod);
    });
  }, [dateRange, supabase]);
