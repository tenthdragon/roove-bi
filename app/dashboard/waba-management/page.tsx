...
export default function WabaManagementPage() {
  ...
  // ── Fetch per-template analytics after templates load ──
  useEffect(() => {
    if (loadingTemplates || templates.length === 0) return;
    const approvedIds = templates
      .filter(t => t.status === 'APPROVED' && !isAutoGenerated(t.name))
      .map(t => t.id);
    ...
    start90.setDate(start90.getDate() - 89); // API enforces strict 90-day max, use 89 for margin
    const start = start90.toISOString().split('T')[0];

    setLoadingTplAnalytics(true);
    fetch(`/api/waba-template-analytics?template_ids=${approvedIds.join(',')}&start=${start}&end=${end}`)
      .then(res => res.json())
      .then(json => {
