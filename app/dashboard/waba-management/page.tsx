...
export default function WabaManagementPage() {
  ...
  const wabaAnalysis = useMemo(() => {
    const byDate: Record<string, { sent: number; delivered: number; orders: number; cost: number; revenue: number; tplRead: number; tplClicked: number; tplReplied: number }> = {};
    ...
    const rows = Object.entries(byDate)
      ...
      .map(([date, v]) => ({
        ...
        readRate: v.delivered > 0 ? (v.tplRead / v.delivered) * 100 : 0,
        clickRate: v.tplRead > 0 ? (v.tplClicked / v.tplRead) * 100 : 0,
        replyRate: v.tplRead > 0 ? (v.tplReplied / v.tplRead) * 100 : 0,
      }));
    ...
    const totals = rows.reduce((acc, r) => ({
      ...
      tplRead: acc.tplRead + r.tplRead,
      tplClicked: acc.tplClicked + r.tplClicked,
      tplReplied: acc.tplReplied + r.tplReplied,
    }), { sent: 0, delivered: 0, orders: 0, cost: 0, revenue: 0, tplRead: 0, tplClicked: 0, tplReplied: 0 });
    ...
    return {
      ...
      totals: {
        ...
        readRate: totals.delivered > 0 ? (totals.tplRead / totals.delivered) * 100 : 0,
        clickRate: totals.tplRead > 0 ? (totals.tplClicked / totals.tplRead) * 100 : 0,
        replyRate: totals.tplRead > 0 ? (totals.tplReplied / totals.tplRead) * 100 : 0,
      },
    };
  }, [adsData, channelData, shipmentCounts, templateAnalyticsDaily]);
        ...
        {loadingAnalytics ? (
          ...
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12, minWidth: 1050 }}>
              <thead>
                <tr style={{ borderBottom: '2px solid #1a2744' }}>
                  {['Date', 'MM Sent', 'MM Delivered', 'Delivery Rate', 'Read Rate', 'Click Rate', 'Reply Rate', 'Order Qty', 'WABA MM Cost', 'Total Purchase', 'Cost/Order'].map(h => (
              ...
              <tbody>
                ...
                <tr style={{ borderTop: '2px solid #1a2744', background: '#0b1121' }}>
                  ...
                  <td style={{ padding: '8px 10px', textAlign: 'right' }}>
                    {wabaAnalysis.totals.tplRead > 0 ? (
                      <span style={{ padding: '2px 7px', borderRadius: 5, fontSize: 10, fontWeight: 700, background: '#1e3a5f', color: '#60a5fa' }}>
                        {wabaAnalysis.totals.readRate.toFixed(1)}%
                      </span>
                    ) : <span style={{ color: '#334155' }}>—</span>}
                  </td>
                  <td style={{ padding: '8px 10px', textAlign: 'right' }}>
                    {wabaAnalysis.totals.tplRead > 0 ? (
                      <span style={{ padding: '2px 7px', borderRadius: 5, fontSize: 10, fontWeight: 700, background: '#1e3a5f', color: '#60a5fa' }}>
                        {wabaAnalysis.totals.clickRate.toFixed(1)}%
                      </span>
                    ) : <span style={{ color: '#334155' }}>—</span>}
                  </td>
                  <td style={{ padding: '8px 10px', textAlign: 'right' }}>
                    {wabaAnalysis.totals.tplRead > 0 ? (
                      <span style={{ padding: '2px 7px', borderRadius: 5, fontSize: 10, fontWeight: 700, background: '#064e3b', color: '#10b981' }}>
                        {wabaAnalysis.totals.replyRate.toFixed(1)}%
                      </span>
                    ) : <span style={{ color: '#334155' }}>—</span>}
                  </td>
