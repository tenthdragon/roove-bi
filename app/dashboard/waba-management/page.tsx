...
export default function WabaManagementPage() {
  ...
  const [formHeaderType, setFormHeaderType] = useState<'NONE' | 'TEXT' | 'IMAGE'>('NONE');
  const [formHeader, setFormHeader] = useState('');
  const [formHeaderImage, setFormHeaderImage] = useState<File | null>(null);
  ...
  useEffect(() => {
    ...
    const url = URL.createObjectURL(formHeaderImage);
  ...
  const headerErrors = useMemo(() => {
    const errs: string[] = [];
    if (formHeaderType !== 'TEXT' || !formHeader) return errs;
  ...
  const headerImageErrors = useMemo(() => {
    const errs: string[] = [];
    if (formHeaderType !== 'IMAGE') return errs;
    if (formCategory === 'AUTHENTICATION') errs.push('Image headers are not supported for authentication templates.');
    if (!formHeaderImage) {
      errs.push('Choose an image for the header.');
      return errs;
    }
    if (!['image/jpeg', 'image/png'].includes(formHeaderImage.type)) {
      errs.push('Image must be a JPG or PNG file.');
    }
    if (formHeaderImage.size > 4 * 1024 * 1024) {
      errs.push('Image must be no larger than 4 MB.');
    }
    return errs;
  }, [formCategory, formHeaderImage, formHeaderType]);
  ...
  const hasValidationErrors = nameErrors.length + headerErrors.length + headerImageErrors.length + bodyErrors.length + footerErrors.length + buttonErrors.length > 0;
  ...
  async function handleCreate() {
    ...
    try {
      ...
      if (formHeaderType === 'TEXT' && formHeader.trim()) {
        components.unshift({ type: 'HEADER', format: 'TEXT', text: formHeader.trim() });
      }
      if (formHeaderType === 'IMAGE') {
        if (!formHeaderImage) throw new Error('Choose an image for the header.');
        const mediaForm = new FormData();
        mediaForm.append('file', formHeaderImage);
        const mediaRes = await fetch('/api/waba-template-media', {
          method: 'POST',
          body: mediaForm,
        });
        const mediaJson = await mediaRes.json();
        if (!mediaRes.ok) throw new Error(mediaJson.error || 'Failed to upload header image');
        components.unshift({
          type: 'HEADER',
          format: 'IMAGE',
          example: { header_handle: [mediaJson.handle] },
        });
      }
      const res = await fetch('/api/waba-templates', {
        method: 'POST',
      ...
      setFormName(''); setFormBody(''); setFormHeaderType('NONE'); setFormHeader(''); setFormHeaderImage(null); setFormFooter(''); setFormButtons([]); setVarSamples({});
            ...
            <div style={{ display: 'flex', gap: 20 }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12, marginBottom: 12 }}>
                  ...
                  <div>
                    ...
                    <select style={selectStyle} value={formCategory} onChange={e => {
                      const category = e.target.value as any;
                      setFormCategory(category);
                      if (category === 'AUTHENTICATION' && formHeaderType === 'IMAGE') {
                ...
                <div style={{ marginBottom: 12 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                    ...
                    {formHeaderType === 'TEXT' && formHeader.length > 0 && <span style={{ fontSize: 10, color: formHeader.length > 60 ? 'var(--red)' : 'var(--text-muted)' }}>{formHeader.length}/60</span>}
                  </div>
                  <select
                    style={{ ...selectStyle, marginBottom: formHeaderType === 'NONE' ? 0 : 8 }}
                    value={formHeaderType}
                    onChange={e => {
                      const type = e.target.value as 'NONE' | 'TEXT' | 'IMAGE';
                      setFormHeaderType(type);
                      if (type !== 'TEXT') setFormHeader('');
                      if (type !== 'IMAGE') setFormHeaderImage(null);
                    }}
                  >
                    <option value="NONE">No header</option>
                    <option value="TEXT">Text header</option>
                    {formCategory !== 'AUTHENTICATION' && <option value="IMAGE">Image header</option>}
                  </select>
                  {formHeaderType === 'TEXT' && (
                  ...
                  {formHeaderType === 'IMAGE' && (
                    <>
                      <input
                        ...
                        onChange={e => setFormHeaderImage(e.target.files?.[0] || null)}
                        style={{ ...inputStyle, padding: 8, borderColor: headerImageErrors.length > 0 ? 'var(--red)' : 'var(--border)' }}
                      />
                      <div style={{ fontSize: 10, color: 'var(--text-muted)', marginTop: 4 }}>JPG or PNG, maximum 4 MB. This image is used as Meta review sample.</div>
                    </>
                  )}
                  {headerErrors.map((e, i) => <div key={i} style={{ color: 'var(--red)', fontSize: 11, marginTop: 4 }}>{e}</div>)}
                  {headerImageErrors.map((e, i) => <div key={i} style={{ color: 'var(--red)', fontSize: 11, marginTop: 4 }}>{e}</div>)}
                </div>
              ...
              <div style={{ width: 320, flexShrink: 0 }}>
                ...
                <div style={{ background: '#e5ddd5', borderRadius: 12, padding: 16, minHeight: 200, position: 'relative' }}>
                  ...
                  <div style={{ position: 'relative', background: '#fff', borderRadius: '0 8px 8px 8px', padding: '8px 10px', maxWidth: '100%', boxShadow: '0 1px 2px rgba(0,0,0,0.13)' }}>
                    {/* Header */}
                    {formHeaderType === 'IMAGE' && formHeaderImageUrl && (
                      <img
                        src={formHeaderImageUrl}
                        alt="Header preview"
                        style={{ width: '100%', maxHeight: 180, objectFit: 'cover', borderRadius: 6, marginBottom: 6, display: 'block' }}
                      />
                    )}
                    {formHeaderType === 'TEXT' && formHeader.trim() && (
                      <div style={{ fontSize: 14, fontWeight: 700, color: '#1a1a1a', marginBottom: 4, lineHeight: 1.3 }}>
                        {formHeader}
                      </div>
                    )}
